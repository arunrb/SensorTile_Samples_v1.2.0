...
void RTC_Handler( RTC_HandleTypeDef *RtcHandle )
{
  ...
  {
    // ARUN
	//  sprintf( dataOut, "\nT: %02d:%02d:%02d.%02d, SystemCoreClock = %d ", stimestructure.Hours, stimestructure.Minutes, stimestructure.Seconds, subSec, SystemCoreClock );
    sprintf( dataOut, "\nT: %02d:%02d:%02d.%02d", stimestructure.Hours, stimestructure.Minutes, stimestructure.Seconds, subSec );
...
* @retval None
*/
void Accelero_Sensor_Handler( void *handle, RTC_HandleTypeDef *RtcHandle )
{

//	RTC_HandleTypeDef * tt = &RtcHandle;
  uint8_t who_am_i;
  float odr;
  float fullScale;
  uint8_t id;
...
//	subSec = (((((( int )RTC_SYNCH_PREDIV) - (( int )stimestructure.SubSeconds)) * 100) / ( RTC_SYNCH_PREDIV + 1 )) & 0xff );
//
//	sprintf( dataOut, "\nT5:%02d:%02d:%02d.%02d, %d,%d,%d", stimestructure.Hours, stimestructure.Minutes, stimestructure.Seconds, subSec, (int)acceleration.AXIS_X, (int)acceleration.AXIS_Y, (int)acceleration.AXIS_Z );

    sprintf( dataOut, "\nX: %d,%d,%d", (int)acceleration.AXIS_X, (int)acceleration.AXIS_Y, (int)acceleration.AXIS_Z );
...
//    if(SendOverUSB) /* Write data on the USB */
//    {
//      sprintf( dataOut, "\n%d, %d, %d", (int)acceleration.AXIS_X, (int)acceleration.AXIS_Y, (int)acceleration.AXIS_Z );
//      CDC_Fill_Buffer(( uint8_t * )dataOut, strlen( dataOut ));
//
//      if ( verbose == 1 )
//      {
...
//        }
//        /*ARUN*/
////        BSP_ACCELERO_Set_ODR_Value(handle, 6660.0); /* ODR 6.66kHz */
//
//        CDC_Fill_Buffer(( uint8_t * )dataOut, strlen( dataOut ));
//
